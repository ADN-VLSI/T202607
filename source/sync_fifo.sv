...
module sync_fifo #(
    ...
    assign full = (WP[ADDR_WIDTH] != RP[ADDR_WIDTH]) &&
                  (WP[ADDR_WIDTH-1:0] == RP[ADDR_WIDTH-1:0]);

    assign data_in_ready_o =
        !full ||
        (data_out_valid_o && data_out_ready_i);
    ...
    assign write_fire = data_in_valid_i && data_in_ready_o;

    assign read_fire = data_out_valid_o && data_out_ready_i;
