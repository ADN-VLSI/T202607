...
task automatic send_data_seq (input logic [7:0] data);
    ...
    // Write data to the TX holding register
    apb_write(ADDR_TXD, {24'h0, data}); 

    // Poll the TX status register until the TX count drops to 0 (transmission done)
    do begin
        apb_read(ADDR_TX_STAT, rdata);
        tx_count = rdata[7:0];
        
        // Optional safety timeout or clock delay to prevent an infinite zero-delay loop
        @(posedge clk); 
    end while (tx_count != 0);

endtask

// 2. Receive Data Sequence

task automatic recv_data_seq (output logic [7:0] data);
    logic [31:0] rdata;
    logic [7:0]  rx_count;

    // Poll the RX status register until the RX count is greater than 0 (data available)
    do begin
        apb_read(ADDR_RX_STAT, rdata);
        rx_count = rdata[7:0];
    ...
    end while (rx_count == 0);

    // Read the received data from the RX data register
